...
% for solving the infinite-horizon optimal control problem for the 
% deterministic linear dyanmics. 
% Please see Section 4.1 of the following article for more details:
%   
...
while i_N <= length(N_set) % iteraton over grid sizes
    ...
    str = sprintf('Ex_1d_Data_N%2.0f',N(1));
...
i_N = length(N_set);
N = N_set(i_N);
str = sprintf('Ex_1d_Data_N%2.0f',N);
load(str)
